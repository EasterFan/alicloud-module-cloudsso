...
resource "alicloud_vswitch" "vswitches" {
  ...
  vswitch_name = length(var.vswitch_cidrs) > 1 || var.use_num_suffix ? format("%s%03d", var.vswitch_name[count.index], count.index + 1) : var.vswitch_name[count.index]
  description  = var.vswitch_description
  tags         = merge(
    {
      Name = format(
        "%s%03d",
        var.vswitch_name[count.index],
        count.index + 1
      )
    },
    var.vswitch_tags,
